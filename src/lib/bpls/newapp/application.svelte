...
{#if mode === "dose"}
  <Page
    ...
    note="Is this place of business rented?"
    {tracking}
  >
...
{#if mode === "yes1"}
  <Page
    title="Business Address"
    {tracking}
    note="Is this place of business located in the LGU operated market or rental space?"
  >
...
{#if mode === "tres"}
  <Page
    title="Business Address"
    note="Please confirm if information is correct"
    {tracking}
  >
    <p>Stall No: U12312</p>
...
{#if mode === "no2"}
  <Page title="Business Address" link="reg" {tracking}>
    <Textfield
      placeholder="Select the place of business from registered lessors"
    />
    <hr class="mt-10 mb-5 " />
    <div>
      <Button
    ...
    <div class="pt-4">
      <p>
        Address not found on the list?<Button
          caption="Click here"
          on:click={() => (mode = "reg")}
          color="primary"
        />
...
{#if mode === "nueve"}
  <Page
    title="Business Address"
    note="Please confirm if information is correct and specify other details"
    {tracking}
  >
...
{#if mode === "reg"}
  <Page
    title="Business Address"
    subtitle="Enter Business Address"
    note="Enter the lessons information and business Address"
    {tracking}
  >
...
{#if mode === "no1"}
  <Page title="Business Address" subtitle="Enter Business Addresss" {tracking}>
    <Textfield
      placeholder="Enter PIN pr latest tax declaration no. of property"
    />
    <Textfield placeholder="Unit No. Bldg No." />
    <Textfield placeholder="Street, Subdivision, barangay" />
    <p class="mt-10 px-2">Select location in map</p>
...
{#if mode === "lob"}
  <Page
    ...
    subtitle="Specify line(s) of Business"
    {tracking}
  >
...
{#if mode === "binfo1"}
  <Page
    ...
    subtitle="Specify Business Information"
    {tracking}
  >
    <Radio
      name="app"
      label="Do you have delivery vans?"
      value=""
      bind:selected={radioValue}
    />
    <Radio
      name="app"
      label="NStore/office located inside centro ?"
      value=""
      bind:selected={radioValue}
    />
    <hr class="mt-10 mb-5 " />
